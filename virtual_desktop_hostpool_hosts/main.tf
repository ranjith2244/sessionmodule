...
resource_group_name = "${var.envi}-wvd-rg"
  host_pool_name = "${var.envi}-wvd-vdp-VirtualDesktopHostPool"
}
...
resource "azurerm_network_interface" "avd_vm_nic" {
  ...
  name                = "${var.envi}-${var.prefix}-${count.index + 1}-nic"
  resource_group_name = local.resource_group_name
...
resource "azurerm_windows_virtual_machine" "avd_vm" {
  count                 = var.rdsh_count
  name                  = "${var.envi}-${var.prefix}-${count.index + 1}"
  resource_group_name   = local.resource_group_name
  location            = var.resource_group_location
  size                  = var.vm_size
  ...
  os_disk {
    name                 = "${lower(var.envi)}--${var.prefix}-${count.index + 1}"
    caching              = "ReadWrite"
